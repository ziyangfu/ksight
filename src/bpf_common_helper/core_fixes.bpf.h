...
static __always_inline __u8 get_inet_sock_freebind(void *inet_sock) {
  ...
  if (bpf_core_field_exists(struct inet_sock___o, freebind))
    return BPF_CORE_READ_BITFIELD_PROBED((struct inet_sock___o *)inet_sock,
                                         freebind);
...
get_inet_sock_bind_address_no_port(void *inet_sock) {
  unsigned long inet_flags;
...
struct cfs_rq___post_v614 {
  unsigned int nr_queued;
} __attribute__((preserve_access_index));

static __always_inline __u8 cfs_rq_get_nr_running_or_nr_queued(void *cfs_rq) {
  struct cfs_rq___pre_v614 *pre = cfs_rq;
  struct cfs_rq___post_v614 *post = cfs_rq;

  if (bpf_core_field_exists(pre->nr_running))
    return BPF_CORE_READ(pre, nr_running);

  return BPF_CORE_READ(post, nr_queued);
}

...
TcprttBpf::TcprttBpf(const ConfigArgs &config) : config_(config) {}

TcprttBpf::~TcprttBpf() { destroy(); }
...
void TcprttBpf::init() {
  ...
  memcpy(skel_->rodata->targ_saddr_v6, config_.laddr_v6,
         sizeof(skel_->rodata->targ_saddr_v6));
  memcpy(skel_->rodata->targ_daddr_v6, config_.raddr_v6,
         sizeof(skel_->rodata->targ_daddr_v6));
  skel_->rodata->targ_ms = config_.milliseconds;

  // 如果系统支持 fentry，就禁用 kprobe 程序的加载

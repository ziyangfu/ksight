...
static __inline bool parse_sk_buff(struct sk_buff *skb,
                                   struct packet_tuple *tuple) {
  if (!skb || !tuple) {
    return false;
  }
...
static int netfilter_handle(struct bpf_nf_ctx *ctx) {
  ...
  __u64 bucket_key;
  if (rule->ip_enable == false && rule->port_enable == false &&
      rule->protocol_enable == false) {
    bucket_key = 0;
  } else {
    bucket_key =
        ((__u64)tuple->dst_ip << 16) | tuple->dst_port | tuple->protocol;
  }
